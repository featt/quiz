...
class QuizQuestionsActivity : AppCompatActivity() {
        ...
        btnSubmit?.setOnClickListener {
            if (!isAnswerChecked) {
                ...
                if (!anyAnswerIsChecked) {
                    Toast.makeText(this, "Пожалуйста, выберите альтернативный вариант", Toast.LENGTH_SHORT).show()
                } else {
                    ...
                    btnSubmit?.text = if (currentQuestionIndex == questionsList.size - 1) "Конец" else "К следующему вопросу"
    ...
    private fun updateQuestion() {
        ...
        btnSubmit?.text = if (currentQuestionIndex == questionsList.size - 1) "Финиш" else "Далее"
    }
